...
3. Each LLM-authored block is normalized to `PracticeBlock` IR:
   ...
   - per-item `display_segments` typed as `math`, `marker`, `korean_label`, or `number_line` so mixed rows can use the correct font and shapes per segment; segments may include `gap_after_in` when source-PDF visual spacing must be preserved
...
- Preserve source PDF layout type first: one-column list, two-column grid, chained equality, table, number-line, diagram, or mixed layout.
...
- Preserve source number-line graphics with `display_segments` number-line entries when the source can be reconstructed from axis domain, ticks, labels, boundary point, endpoint openness, and interval direction. This is a formal `pdf2concept-problem-pptx.llm-ir.v1` display segment kind:
  - `kind`: `number_line`
  - `line_index`: the display row where the number line should appear
  - `text`: semicolon-separated key/value spec
  - supported `text` keys: `min`, `max`, `point`, `direction`, `closed`, `ticks`, `labels`, `blank`
  - `min`/`max`: numeric visible axis domain
  - `point`: numeric boundary value for interval number lines
  - `direction`: `left` or `right` for the interval direction from `point`
  - `closed`: `true` for a filled endpoint dot, `false` for an open endpoint dot
  - `ticks`: comma-separated numeric tick positions
  - `labels`: comma-separated tick labels to display; keep them aligned to `ticks`
  - `blank=true`: draw only the undecorated axis/ticks/labels for student marking; omit `point`, `direction`, and `closed`
- Example source interval with an open endpoint at `-3` and shading to the right: `{"kind":"number_line","text":"min=-4;max=1;point=-3;direction=right;closed=false;ticks=-3;labels=-3","line_index":0}`. Example blank axis for `x≥4`: `{"kind":"number_line","text":"min=1;max=5;blank=true;ticks=1,2,3,4,5;labels=1,2,3,4,5","line_index":1}`.
- Number-line geometry is layout data, not problem text. Keep the item `raw_text` semantic, keep `blank_count` at `0` unless the source also has actual square blanks, and render the number line as editable PowerPoint shapes inside the item reveal group.
