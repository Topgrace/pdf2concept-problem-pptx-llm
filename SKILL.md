...
- Preserve number-line layouts as editable `display_segments` rather than screenshots whenever the line can be described by an axis, ticks, endpoint, and direction. Use `{"kind":"number_line","text":"min=-4;max=1;point=-3;direction=right;closed=false;ticks=-3;labels=-3","line_index":0}` for an open dot at `-3` with the shaded/arrowed interval to the right, and `{"kind":"number_line","text":"min=2;max=7;point=5;direction=left;closed=false;ticks=5;labels=5","line_index":0}` for an open dot at `5` with the interval to the left.
- The `number_line` segment text is a semicolon-separated key/value spec in schema `pdf2concept-problem-pptx.llm-ir.v1`. Supported keys are `min` and `max` for the visible axis domain; `point` for the boundary value; `direction=left|right` for the interval direction; `closed=true|false` for filled or open endpoint dots; `ticks` for comma-separated tick positions; `labels` for comma-separated tick labels to show; and `blank=true` for an undecorated number line that students must mark themselves. Omit `point`, `direction`, and `closed` when `blank=true`.
- For number-line items, keep `raw_text` semantic and concise, such as `(1) 수직선: 열린 점 -3, 오른쪽 영역` or `(5) x≥4`. Do not encode number-line arrows as typed arrow characters, do not create `[ ]` tokens for number-line drawing space, and keep `blank_count` at `0` unless the source also contains actual square blanks outside the number line.
...
- Render `display_segments` number lines as editable PowerPoint objects: horizontal axis line, arrowheads, tick marks, optional labels, optional interval band, optional endpoint dot, and optional interval arrow. Open endpoints use an unfilled circle; closed endpoints use a filled circle. The number line must remain inside the same reveal group as its problem number and any associated formula text.
...
- For source number lines, classify the boundary point, whether the endpoint is open or closed, the interval direction, and the visible tick labels. Write that geometry as a `display_segments` entry with `kind: "number_line"` using the semicolon key/value spec above.
